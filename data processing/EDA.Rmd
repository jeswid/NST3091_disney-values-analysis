...
```{r}
#running time visualisation (histogram)
# Histogram of Running Time using plotly
p4 <- plot_ly(disney_df, x = ~running_time, type = "histogram", marker = list(color = 'lightcoral')) %>%
  layout(title = "Histogram of Running Time", xaxis = list(title = "Running Time (minutes)"), yaxis = list(title = "Count"))

htmlwidgets::saveWidget(p4, file = "runtime_hist.html")
```
...
```{r}
#plot the number of movies produced by country (not too meaningful)
movies_by_countries <- disney_df %>% count(country)

movies_by_countries %>% 
  ggplot(aes(x = fct_reorder(country, n, .desc = TRUE), y = n)) +
  geom_col() +
  geom_text(aes(label = n), vjust = -0.5) +  # Adds labels on top of the bars
